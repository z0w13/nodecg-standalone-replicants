...
const REPLICANT_HANDLER = {
  ...
  set (target, prop, newValue) {
    ...
    if (newValue === target[prop]) {
      target.log.log('value unchanged, no action will be taken')
      return true
    }

    target.validate(newValue)
    target.log.log('running setter with', newValue)
...
class Replicant extends EventEmitter {
  ...
  _handleAssignment (data) {
    ...
    this.log.log('received replicantAssigned', data)
  ...
  _handleOperations (data) {
    ...
    this.log.log('received replicantOperations', data)
   ...
   */
  _fullUpdate () {
    shared.readReplicant(this._socket, this.name, this.namespace, (data) => {
