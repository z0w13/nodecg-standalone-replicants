...
test.serial(
  ...
  (t) => {
    ...
    const mgr = new Manager(sock, 'test-bundle')

    const rep1 = mgr.newReplicant('clientDupRef')
    const rep2 = mgr.newReplicant('clientDupRef')
...
test.serial('should only apply defaultValue when first declared', async (t) => {
  ...
  mgr.newReplicant('clientTest', {
    persistent: false,
    defaultValue: 'foo'
  })

  const ret = await new Promise((resolve) => {
    const rep = mgr.newReplicant('clientTest', {
      defaultValue: 'bar'
    })
    rep.on('declared', () => {
      resolve(rep.value)
...
test.serial('should be assignable via the ".value" property', async (t) => {
  ...
  const ret = await new Promise((resolve) => {
    const rep = mgr.newReplicant('clientAssignmentTest', {
      persistent: false
    })
...
test.serial(
  ...
  async (t) => {
    ...
    const ret = await new Promise((resolve) => {
      const rep = mgr.newReplicant('clientUndefinedChangeTest', {
        persistent: false
      })
      rep.on('change', () => {
        resolve({
...
//  (t) => {
//    const rep = t.context.apis.extension.Replicant('serverArrInsertObj', {
//      defaultValue: []
//    })
...
//      () =>
//        new Promise((resolve) => {
//          window.s2c_nestedArrTest = window.dashboardApi.Replicant(
//            's2c_nestedArrTest'
//          )
//          window.s2c_nestedArrTest.on('declared', () => {
//            resolve()
//
//            window.s2c_nestedArrTest.on(
//              'change',
//              (newVal, oldVal, operations) => {
//                if (newVal && oldVal && operations) {
...
//        C.replicantsRoot(),
//        'test-bundle/clientPersistence.rep'
//      )
...
//      () =>
//        new Promise((resolve) => {
//          window.clientChangePersistence = window.dashboardApi.Replicant(
//            'clientChangePersistence'
//          )
...
//        const replicantPath = path.join(
//          C.replicantsRoot(),
//          'test-bundle/clientChangePersistence.rep'
//        )
...
//      const replicantPath = path.join(
//        C.replicantsRoot(),
//        'test-bundle/clientFalseyWrite.rep'
//      )
//      fs.readFile(replicantPath, 'utf-8', (err, data) => {
//        if (err) {
//          throw err
...
//  const replicantPath = path.join(
//    C.replicantsRoot(),
//    'test-bundle/clientTransience.rep'
//  )
